...
class Products extends Table {
  ...
  TextColumn get sku => text().nullable()(); // Added SKU
  ...
  RealColumn get mrp => real().nullable()(); // Added MRP
  TextColumn get discountType => text().nullable()(); // Added Discount Type (Percentage/Flat)
  RealColumn get discountValue => real().withDefault(const Constant(0.0))(); // Added Discount Value
  ...
  BoolColumn get trackInventory => boolean().withDefault(const Constant(true))(); // Added Track Inventory
  RealColumn get reorderLevel => real().withDefault(const Constant(0.0))(); // Added Reorder Level
  RealColumn get reorderQuantity => real().withDefault(const Constant(0.0))(); // Added Reorder Quantity
  TextColumn get warehouse => text().nullable()(); // Added Warehouse
  TextColumn get unit => text().withDefault(const Constant('pcs'))();
  TextColumn get hsnSac => text().nullable()(); // GST support
  TextColumn get taxClass => text().nullable()(); // Added Tax Class
  RealColumn get gstRate => real().withDefault(const Constant(0.0))(); // 0, 5, 12, 18, 28
  TextColumn get imageUrl => text().nullable()(); // Keep for backward compatibility or as primary image
  TextColumn get images => text().nullable()(); // Added for multiple images (JSON string)
  TextColumn get description => text().nullable()(); // Added Description
  TextColumn get warrantyPeriod => text().nullable()(); // Added Warranty
  TextColumn get expiryPeriod => text().nullable()(); // Added Expiry
  TextColumn get tags => text().nullable()(); // Added Tags
  TextColumn get status => text().withDefault(const Constant('Active'))(); // Added Status (Active/Inactive)
  DateTimeColumn get createdAt => dateTime().withDefault(currentDateAndTime)();
